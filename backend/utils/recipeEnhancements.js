const { estimateNutrition } = require("./nutritionEstimator");
...
const enhanceRecipe = (recipe = {}) => {
  ...
  const normalizedNutrition = {
    calories: Math.max(0, Math.round(Number(nutrition.calories) || 0)),
    protein: Math.max(0, Math.round(Number(nutrition.protein) || 0)),
    carbs: Math.max(0, Math.round(Number(nutrition.carbs) || 0)),
    fat: Math.max(0, Math.round(Number(nutrition.fat) || 0)),
    fiber: Math.max(0, Math.round(Number(nutrition.fiber) || 0))
  };
  const estimatedNutrition = estimateNutrition(ingredients, servings);
  const hasEstimatedNutrition = Object.values(estimatedNutrition).some((value) => Number(value) > 0);
  ...
  return {
    ...
    nutrition: hasEstimatedNutrition ? estimatedNutrition : normalizedNutrition,
    healthScore,
